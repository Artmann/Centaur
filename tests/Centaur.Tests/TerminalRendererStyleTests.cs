...
public class TerminalRendererStyleTests
{
    ...
    // A horizontal slice of a cell's height, as fractions from its top.
    readonly record struct Band(float from, float to);

    // Ink pixels in the given horizontal band of a cell.
    static int InkInBand(SKBitmap bitmap, TerminalRenderer renderer, int col, int row, Band band)
    {
        ...
        var y0 = Math.Max(0, (int)(top + band.from * renderer.cellHeight));
        var y1 = Math.Min(bitmap.Height, (int)(top + band.to * renderer.cellHeight));
    ...
    public void Underline_DrawsInkBelowTheGlyph()
    {
        ...
        var lower = InkInBand(bitmap, renderer, 0, 0, new Band(0.6f, 1.0f));
    ...
    public void UnderlineVariants_DrawInk(string sgr)
    {
        ...
        {
            ink += InkInBand(bitmap, renderer, col, 0, new Band(0.5f, 1.0f));
        }
    ...
    public void Strikethrough_DrawsInkAcrossTheMiddle()
    {
        ...
        var middle = InkInBand(bitmap, renderer, 0, 0, new Band(0.3f, 0.75f));
    ...
    public void Overline_DrawsInkAtTheTop()
    {
        ...
        var top = InkInBand(bitmap, renderer, 0, 0, new Band(0.0f, 0.35f));
